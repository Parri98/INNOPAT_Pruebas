...
(() => {
  ...
  const state = {
    hitos: [],
    activeIndex: 0,
    visited: new Set(JSON.parse(localStorage.getItem("innopat.visited") || "[]")),
    prefs: JSON.parse(localStorage.getItem("innopat.prefs") || "{}"),
    lang: localStorage.getItem("innopat.lang") || "es",
    googleMap: null,
    googleMarkers: [],
    userMarker: null,
    accuracyCircle: null,
  ...
  function toggleViews(view) {
    ...
    $("#googleMapView").hidden = selected !== "google";
    $("#mapView").hidden = selected !== "plan";
    const pairs = [
      ["#listViewBtn", selected === "list"],
      ["#googleMapViewBtn", selected === "google"],
    ...
    if (selected === "google") {
      initGoogleMap();
      history.replaceState(null,"","#google-map-section");
  ...
  async function startQrScanner() {
    ...
    if (typeof Html5Qrcode === "undefined") {
      status.textContent = "No se pudo cargar el módulo lector QR. Comprueba la conexión.";
      return;
    }
    try {
  ...
  async function scanQrFile(file) {
    if (!file) return;
    if (typeof Html5Qrcode === "undefined") {
      $("#qrStatus").textContent = "No se pudo cargar el lector QR.";
      return;
    }
    try {
  ...
  function bind() {
    ...
    $("#googleMapViewBtn").addEventListener("click", () => toggleViews("google"));
    ...
    $("#centerSiteBtn").addEventListener("click", async () => { const map = await initGoogleMap(); if (map) { map.panTo(config.siteCenter); map.setZoom(18); } });
    ...
    $("#language").addEventListener("change", e => {
      ...
      localStorage.setItem("innopat.lang", state.lang);
      applyLanguage();
    });
    ...
    [$("#hitoDialog"),$("#a11yPanel"),$("#noticeDialog"),$("#qrDialog")].forEach(dialog => {
